...
async function createEmployee(data) {
    ...
    if (!res.ok) {
        ...
        // If error says uniqueness constraint, we might assume user exists.
        console.warn(`Warning creating employee ${data.fullName}: ${err}`);
        return null;
    }
...
async function createSchedule(employeeId, start, end, note) {
    ...
    if (!res.ok) {
        console.warn(`Warning creating schedule: ${await res.text()}`);
        return;
    }
    console.log(`  ✅ Schedule added`);
}

async function createRecurring(employeeId, patterns) {
    ...
    if (!res.ok) {
        console.warn(`Warning creating recurring: ${await res.text()}`);
        return;
    }
